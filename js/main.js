...
async function init() {
    ...
    await sleep(1000)
    node.innerText = ""
    await node.type('Coding is , ')

    while (true) {
        await node.type('Cool!')
        await sleep(2000)
        await node.delete('Cool!')
        await node.type('Art!')
        await sleep(2000)
...
liFilter.forEach(li => {
    li.onclick = function () {
        ...
        imgs.forEach(img => {
            ...
            if (img.getAttribute('data-filter') == li.textContent.toLowerCase() || li.textContent == "All") {
